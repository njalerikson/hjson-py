...
from hjson.compat import PY3, b
...
class TestScanString(TestCase):
    ...
    def _test_scanstring(self, scanstring):
        if sys.maxunicode == 65535:
            self.assertEqual(
                scanstring(u'"z\U0001d120x"', 1, None, True), (u"z\U0001d120x", 6)
            )
        else:
            self.assertEqual(
                scanstring(u'"z\U0001d120x"', 1, None, True), (u"z\U0001d120x", 5)
            )
        ...
        self.assertEqual(
            scanstring('["Unclosed array"', 2, None, True), (u"Unclosed array", 17)
        )
        ...
        self.assertEqual(
            scanstring('{"Extra comma": true,}', 2, None, True), (u"Extra comma", 14)
        )
        ...
        self.assertEqual(
            scanstring('{"Missing colon" null}', 2, None, True), (u"Missing colon", 16)
        )

        self.assertEqual(
            scanstring('{"Double colon":: null}', 2, None, True), (u"Double colon", 15)
        )
        ...
        self.assertEqual(
            scanstring('["Bad value", truth]', 2, None, True), (u"Bad value", 12)
        )
