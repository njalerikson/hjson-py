...
class TestAssets(TestCase):
    ...
    def check(self, name, file, inputCr):
        ...
        try:
            ...
            # dbg
            # with open(name + "_dbg1.txt", "w") as tmp: tmp.write(hjson1.encode("utf-8"))  # noqa: B950
            # with open(name + "_dbg2.txt", "w") as tmp: tmp.write(hjson2.encode("utf-8"))  # noqa: B950
            # with codecs.open(name + "_dbg3.txt", 'w', 'utf-8') as tmp: hjson.dump(data, tmp)  # noqa: B950
    ...
    def test_files(self):
        for file in self.assets:
            ...
            if name.startswith("stringify/quotes") or name.startswith("extra/"):
                continue  # ignore/not supported
