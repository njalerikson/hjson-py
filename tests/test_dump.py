...
from hjson.compat import PY3, StringIO, b, binary_type, long_type
...
class TestDump(TestCase):
    ...
    def test_stringify_key(self):
        ...
        for k, expect in items:
            self.assertEqual(json.loads(json.dumpsJSON({k: expect})), {expect: expect})
            self.assertEqual(
        ...
        for v in [{}, {"other": 1}, {b("derp"): 1, "herp": 2}]:
            for sort_keys in [False, True]:
                ...
                self.assertEqual(
                    json.loads(
                        json.dumpsJSON({"": v0}, skipkeys=True, sort_keys=sort_keys)
                    ),
                    {"": v1},
                )
                self.assertEqual(
    ...
    def test_encode_truefalse(self):
        self.assertEqual(
            json.dumpsJSON({True: False, False: True}, sort_keys=True),
            '{"false": true, "true": false}',
        )
        self.assertEqual(
            json.dumpsJSON(
                {2: 3.0, 4.0: long_type(5), False: 1, long_type(6): True, "7": 0},
                sort_keys=True,
            ),
